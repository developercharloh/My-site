...
const CACHE_NAME = 'deriv-bot-v44-2026-05-12-mobile-unblock';
const OFFLINE_URL = '/offline.html';

// Files to cache immediately on install. We deliberately do NOT precache
// the root navigation HTML so updates are always picked up from the network.
...
self.addEventListener('fetch', event => {
    ...
    // Skip everything under /dtrader/* — that subtree is a vendored
    // standalone app (DTrader from the deriv-app fork) and must NOT be
    // proxied or cached by Apollo's service worker. Let the network
    // serve its own assets unaltered.
    if (url.pathname === '/dtrader' || url.pathname.startsWith('/dtrader/')) {
        return;
    }
