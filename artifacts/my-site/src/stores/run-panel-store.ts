...
export default class RunPanelStore {
    ...
    constructor(root_store: RootStore, core: TStores) {
        makeObservable(this, {
            ...
            virtual_phase: observable,
            onVirtualPhaseEvent: action,
            digit_stats: observable,
            onDigitStatsEvent: action,
    ...
    virtual_phase: 'idle' | 'virtual' | 'real_recovery' = 'idle';
    digit_stats: { even_pct: number; odd_pct: number; n: number } = { even_pct: 0, odd_pct: 0, n: 0 };
    ...
    get is_stop_button_disabled() {
        // The Stop button must NEVER be disabled while the bot is running.
        // Previously PURCHASE_SENT (every trade) and IS_STOPPING were in this list,
        // which made the button unreachable precisely when users need it most.
        return false;
    }
    ...
    onRunButtonClick = async () => {
        ...
        runInAction(() => {
            ...
            this.dbot.runBot();
            // Auto-navigate to Journal so the user sees live output immediately
    ...
    stopBot = () => {
        ...
        // ── Step 1: Kill all event listeners FIRST ──────────────────────────
        // terminateSession() inside dbot.stopBot() fires globalObserver.emit('bot.stop')
        // SYNCHRONOUSLY inside a Promise constructor — before any await. If listeners
        // are still registered, onBotStopEvent runs with the RESTARTONERROR path
        // (Elite Entry Scanner Bot has shouldRestartOnError:true), which leaves
        // is_running=true and the Stop button stays stuck. Unregistering first
        // ensures bot.stop fires into empty space and can never restart the bot.
        this.unregisterBotListeners();

        // ── Step 2: Reset ALL UI state immediately ───────────────────────────
        this.error_type = undefined;
        this.is_sell_requested = false;
        this.virtual_phase = 'idle';
        ...
        document.dispatchEvent(new Event('ListenPWAUpdate'));
        ...
        // ── Step 3: Tell the engine to stop (async, fires bot.stop sync but no listener) ──
        this.dbot.stopBot();
    };
    ...
    registerBotListeners = () => {
        const { portfolio, summary_card, transactions } = this.root_store;
        ...
        observer.register('bot.contract', portfolio.onBotContractEvent);
        observer.register('bot.virtual_transaction', transactions.pushVirtualTransaction);
        observer.register('bot.virtual_phase', this.onVirtualPhaseEvent);
        observer.register('bot.digit_stats', this.onDigitStatsEvent);
    ...
    onVirtualPhaseEvent = ({ phase }: { phase: 'idle' | 'virtual' | 'real_recovery' }) => {
        this.virtual_phase = phase;
    };

    onDigitStatsEvent = ({ even_pct, odd_pct, n }: { even_pct: number; odd_pct: number; n: number }) => {
        this.digit_stats = { even_pct, odd_pct, n };
    };
    ...
    onBotStopEvent = () => {
        ...
        if (this.error_type === ErrorTypes.RECOVERABLE_ERRORS) {
            ...
            // Bot stopped — current contract just closed or was cancelled by user.
            this.error_type = undefined;
            this.is_sell_requested = false;
            ...
            this.unregisterBotListeners();
            self_exclusion.resetSelfExclusion();
            // Mark bot as no longer running so the Run button reappears and
            // the Stop button is hidden. Without this, is_running stays true
            // permanently and the UI freezes on the Stop state.
            this.setIsRunning(false);
        }
    ...
    unregisterBotListeners = () => {
        ...
        observer.unregisterAll('bot.sell');           // was missing — orphaned listener
        observer.unregisterAll('bot.stop');
        observer.unregisterAll('bot.bot_ready');      // was missing — orphaned listener
        ...
        observer.unregisterAll('bot.virtual_transaction');
        observer.unregisterAll('bot.virtual_phase');
        observer.unregisterAll('bot.digit_stats');
        observer.unregisterAll('Error');
        observer.unregisterAll('bot.recoverOpenPositionLimitExceeded'); // was missing
    };
