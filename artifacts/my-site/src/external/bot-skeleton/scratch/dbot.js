...
class DBot {
    ...
    async stopBot() {
        ...
        // Always force-stop the interpreter regardless of api_base.is_stopping.
        // The old guard (`if (!api_base.is_stopping)`) could skip interpreter.stop()
        // when the bot had internally restarted after an error, leaving $scope.stopped
        // as false and the interpreter loop running indefinitely.
        api_base.is_stopping = false; // reset so interpreter.stop() proceeds unconditionally
