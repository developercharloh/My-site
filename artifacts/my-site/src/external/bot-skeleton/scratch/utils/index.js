...
export const validateErrorOnBlockDelete = () => {
    ...
    const required_block_types = [mandatory_trade_option_block, 'trade_definition', 'purchase', 'apollo_purchase', 'apollo_purchase2', 'vh_normal_purchase', 'vh_purchase', 'vh_reset_purchase', 'viper_purchase', 'before_purchase'];
...
const getMissingBlocks = (workspace, required_block_types) => {
    ...
    return required_block_types.filter(blockType => {
        if (blockType === 'purchase') {
            return !all_blocks.some(block => PURCHASE_BLOCK_ALIASES.includes(block.type));
        }
        return !all_blocks.some(block => block.type === blockType);
    });
};
...
const getDisabledBlocks = required_blocks_check => {
    ...
    const mandatory_blocks = ['before_purchase', 'purchase', 'apollo_purchase', 'apollo_purchase2', 'trade_definition', 'trade_definition_tradeoptions', 'viper_purchase'];
