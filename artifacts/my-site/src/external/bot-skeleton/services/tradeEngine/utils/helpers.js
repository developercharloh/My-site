...
export const tradeOptionToProposal = (trade_option, purchase_reference) =>
    trade_option.contractTypes.map(type => {
        ...
        const NO_PREDICTION_TYPES = ['TICKLOW', 'TICKHIGH', 'DIGITEVEN', 'DIGITODD'];
        if (trade_option.prediction !== undefined && !NO_PREDICTION_TYPES.includes(type)) {
            proposal.selected_tick = trade_option.prediction;
        }
        if (!NO_PREDICTION_TYPES.includes(type) && trade_option.prediction !== undefined) {
...
export const tradeOptionToBuy = (contract_type, trade_option) => {
    ...
    const NO_PREDICTION_BUY_TYPES = ['TICKLOW', 'TICKHIGH', 'DIGITEVEN', 'DIGITODD'];
    if (trade_option.prediction !== undefined && !NO_PREDICTION_BUY_TYPES.includes(contract_type)) {
        buy.parameters.selected_tick = trade_option.prediction;
    }
    if (!NO_PREDICTION_BUY_TYPES.includes(contract_type) && trade_option.prediction !== undefined) {
