...
import { info, log, notify } from '../utils/broadcast';
...
export default Engine =>
    class Total extends Engine {
        ...
        updateTotals(contract) {
            ...
            const digitContractTypes = ['DIGITOVER', 'DIGITUNDER', 'DIGITEVEN', 'DIGITODD'];
            if (contract.contract_type && digitContractTypes.includes(contract.contract_type)) {
                const exitVal = String(contract.exit_tick_display_value ?? '');
                const lastDigit = exitVal.slice(-1);
                let entryPoint;
                switch (contract.contract_type) {
                    case 'DIGITOVER':  entryPoint = `OVER ${contract.barrier}`;  break;
                    case 'DIGITUNDER': entryPoint = `UNDER ${contract.barrier}`; break;
                    case 'DIGITEVEN':  entryPoint = 'EVEN';                      break;
                    case 'DIGITODD':   entryPoint = 'ODD';                       break;
                    default:           entryPoint = contract.contract_type;
                }
                notify('journal__text--digit-info', `Entry: ${entryPoint}  |  Last digit: ${lastDigit}`);
            }
        }
