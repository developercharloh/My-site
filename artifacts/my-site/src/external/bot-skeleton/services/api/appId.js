...
export const generateDerivApiInstance = () => {
    ...
    const cleanedAppId = getNumericAppId();
...
export const V2GetActiveToken = () => {
    // Try the gate-specific token first, fall back to generic authToken
    const gate = localStorage.getItem('login_gate');
    if (gate) {
        const gateToken = localStorage.getItem(`authToken_${gate}`);
        if (gateToken && gateToken !== 'null') return gateToken;
    }
