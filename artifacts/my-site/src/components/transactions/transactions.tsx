...
import { TVirtualTransaction } from '@/stores/transactions-store';
import { DerivLightEmptyCardboardBoxIcon } from '@deriv/quill-icons/Illustration';
import { Localize } from '@deriv-com/translations';
import { useDevice } from '@deriv-com/ui';
import ThemedScrollbars from '../shared_ui/themed-scrollbars';
import Transaction, { VirtualTransaction } from './transaction';
...
const TransactionItem = ({ row = false, onClickTransaction, active_transaction_id }: TTransactionItem) => {
    switch (row.type) {
        ...
        case transaction_elements.VIRTUAL_HOOK: {
            return <VirtualTransaction data={row.data as TVirtualTransaction} />;
        }
...
const Transactions = observer(({ is_drawer_open }: TTransactions) => {
    ...
    const { contract_stage, digit_stats, is_clear_stat_disabled, onClearStatClick } = run_panel;
    ...
    return (
        ...
        >
            ...
            {digit_stats.n > 0 && (
                <div className='transactions__digit-stats'>
                    <span className='transactions__digit-stats-even'>
                        Even 🟢: {digit_stats.even_pct}%
                    </span>
                    <span className='transactions__digit-stats-sep'> || </span>
                    <span className='transactions__digit-stats-odd'>
                        Odd 🔴: {digit_stats.odd_pct}%
                    </span>
                    <span className='transactions__digit-stats-label'>
                        &nbsp;Based on last {digit_stats.n} ticks
                    </span>
                </div>
            )}
                        ...
                        <DataList
                            ...
                            keyMapper={row => {
                                switch (row.type) {
                                    ...
                                    case transaction_elements.VIRTUAL_HOOK: {
                                        return `vh_${row.data.timestamp}_${row.data.trade_number}`;
                                    }
                                    case transaction_elements.DIVIDER: {
                                        return row.data;
                                    }
