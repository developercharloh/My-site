...
export const AuthWrapper = () => {
    ...
    // If there are no auth params in the URL, there's nothing to wait for —
    // render the App immediately. Only show the spinner when we actually
    // need to exchange tokens for a session.
    const needsAuthInit = loginInfo.length > 0;
    const [isAuthComplete, setIsAuthComplete] = React.useState(!needsAuthInit);
    const { isOnline } = useOfflineDetection();

    React.useEffect(() => {
        if (!needsAuthInit) return;
        ...
        // Hard cap so the spinner never hangs longer than ~5s if the
        // authorize call stalls — App will mount and recover from any state.
        const hardTimeout = setTimeout(() => setIsAuthComplete(true), 5000);

        initializeAuth().finally(() => clearTimeout(hardTimeout));

        return () => clearTimeout(hardTimeout);
    }, [loginInfo, paramsToDelete, isOnline, needsAuthInit]);
